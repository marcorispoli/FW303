...
static volatile FRONT_MOTOR_t frontMotor; //!< Motor main structure variable declaration
...
#define MOTOR_ID MOTOR_M1
#define MOTOR_STEP_SET uc_STEP_M1_Set()
#define MOTOR_STEP_CLEAR uc_STEP_M1_Clear()
#define OPTO_GET OPTO_F_Get()
...
void motorFrontTest(void){
    static bool button_stat = false;
    
    if(button_stat != uc_TEST_PUSH_Get()){
        button_stat = uc_TEST_PUSH_Get();
        if(button_stat) activateFrontCollimation();        
    }        
}

void activateFrontCollimation(void){
